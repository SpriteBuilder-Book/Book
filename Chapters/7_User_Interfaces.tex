...
sprite, so that is rendered ontop of the background image:
\begin{figure}[H]
    \centering
...
the node library \textbf{below} the \textit{background} node in the timeline
(so that the scroll view is rendered in front of the background image). 

Set the position to (0,0). 

The scroll view shall cover the entire screen, so set the size type to
\textit{percentage of parent container} and choose 100\% for the width and the height of the scroll view. 

Set up the scroll view specific settings in the
...
scroll view and the label we added earlier.
...
\begin{details}[The Delegate pattern]
...
With \inlinecode{isDraggingPot} disabled, the code in \inlinecode{touchMoved}
...
Let's implement \inlinecode{presentGameOverPopup} now. There aren't a lot of new
