...
implement:
\begin{itemize}
  \item When we start a game, the \inlinecode{Gameplay} class needs to know
  which game mode a user has selected
  \item \inlinecode{Gameplay} shouldn't know
  anything about the rules of the selected game mode. That way we could add game
  modes in the future without increasing the complexity of \inlinecode{Gameplay}
...
\inlinecode{Gameplay} class. In this scenario, the \inlinecode{Gameplay} class
...
Let's start implementing this design. We should start by defining a protocol
that can be implemented by all game modes.
\begin{leftbar}
Create a new Swift file in \xcode{}. Then add the following protocol definition
to it:
\begin{lstlisting}
protocol GameModeDelegate: class {
  ...
  func gameplayStep(delta: CCTime) -> Bool
}
\end{lstlisting}
\end{leftbar}
...
% code
Let's discuss this protocol in detail. The first thing to note is that we
have defined the protocol as a class-only protocol \ldots \cocos{} needs to
store a reference to this game mode, that is only possible if the type
implementing this protocol is a class.


First, we require on property:
\inlinecode{userInterface}. This property shall provide access to the
user interface of the game mode to outside classes. The \inlinecode{Gameplay}
class can use this property to access the UI for the currently active game mode.
We only expose as a getter as part of the protocol, the UI cannot be changed
from outside of the game mode class.
