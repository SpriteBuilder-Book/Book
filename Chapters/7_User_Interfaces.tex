...
\inlinecode{NSObject} to make this class visible to Objective-C.
...
are derived from \inlinecode{NSObject}, therefore explicitly subclassing from
\inlinecode{NSObject} wasn't necessary.
The \inlinecode{TimedGameMode} class is our first class that is not a subclass
of a \cocos{} class.
...
\end{lstlisting}
\end{leftbar}
In the first line after the call to \inlinecode{super}
(which is \inlinecode{NSObject}) we load the \filemention{TimedModeUI}
\ccbfile{}.
We store the loaded node hierarchy in the \inlinecode{userInterface} property, that way it
...
from \inlinecode{NSObject} to expose this class to our Objective-C code.
We declare two properties for the two UI elements in the \filemention{EndlessModeUI} \ccbfile{}.
