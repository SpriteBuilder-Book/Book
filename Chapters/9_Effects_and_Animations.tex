...
\inlinecode{performCaughtStep(fallingObject:)} method.
...
There's one issue with timelines that we haven't discussed yet. \textbf{Only one
...
sub-\ccbfile{} within \filemention{MainScene.ccb}. However, there is luckily a
...
\begin{leftbar}
Modify the \inlinecode{performMissedStep} method to look as following:
\begin{lstlisting}
...
\inlinecode{OALSimpleAudio.sharedInstance()}. Then we find the file path to the
audio file we want to play. Note that we need to use the
\inlinecode{fullPathForFilenameIgnoringResolutions} method. Typically
\inlinecode{CCFileUtils} will look for our file inside of a path that is
...
You can see it in the folder structure of the \textit{Published-iOS} folder,
...
Once we have the file path, we simply call the \inlinecode{playEffect} method.
...
Instead of calling \inlinecode{playEffect}, we call \inlinecode{preloadEffect}
